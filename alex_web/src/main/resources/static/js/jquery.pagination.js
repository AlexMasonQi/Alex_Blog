...
jQuery.fn.pagination = function(maxentries, opts){
	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			var pagetext = '<input id="pagevalue" size="1" value="'+(current_page+1)+'"type="text" onkeyup="value=value.replace(/[^\\d]/g,\'\')">';
            var toPage='<a id="search" href="#">GO</a>';
            $(pagetext).appendTo(panel);
            $(toPage).appendTo(panel);
            $("#search").bind("click",function(evt){
                var iPageNum = $.trim($("#pagevalue").val()) -1;
                if(iPageNum<np && iPageNum>-1){pageSelected(iPageNum,evt);}else{layer.alert("页数不存在");}
            });
            /****************** Added End ******************/
		}
